...
one final pass for clarity, coverage, CI, and cleanup.

## What it does

This customization treats "ready for review" as a real checklist, not just a GitHub button. It checks
the changed content, tests, description, CI state, and commit history before handing the PR over.
...
5. Confirm the PR description still matches the actual diff.
```
...
```md
Before marking a draft PR ready, confirm required checks are green, the description matches the
current diff, and the commit history is clean enough for review.
```
